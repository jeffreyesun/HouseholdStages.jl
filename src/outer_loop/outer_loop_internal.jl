...
"""
Sup-norm of `new − old`, skipping non-finite differences. A fixed feasibility mask seats
`-Inf` on infeasible cells (see `BorrowingConstraintStage`); that set is constant across a
fixed-`env` solve, so the `-Inf − (-Inf) = NaN` it would otherwise produce must not poison the
convergence metric — those cells contribute `0`. On all-finite arrays this is exactly
`maximum(abs, new .- old)`.
"""
function _sup_norm_diff(new, old)
    d = new .- old
    return mapreduce(x -> isfinite(x) ? abs(x) : zero(x), max, d; init = zero(eltype(d)))
end
...
"""
Spec/Buffer-keyed VFI: backward-iterate `V` to its fixed point at `env`. Errors on
non-convergence.

`env` is constant across the iterations, so only the first `backward!` runs with
`env_changed = true` (end-goal §5.3) to seat the env-dependent fields; later iterations
pass `env_changed = false` to skip the redundant refills. The first-iteration refill is
not optional: the buffer may have been seated for a *different* `env` on a prior call
(Jacobian FD perturbs `env` and re-solves on the same buffer).
"""
function solve_vfi_steady_state_given_env!(spec::AbstractStageSpec, env,
    ...
    iters = 0
    while diff > tol
        V_new = backward!(buffer, spec, V, env; env_changed = iters == 0)
        diff  = _sup_norm_diff(V_new, V)
        V .= V_new
        iters += 1
...
"""
Spec/Buffer-keyed Λ-iteration: forward-iterate `Λ ← forward!(Λ)` to its stationary
distribution. Kernels must have been seated by a prior `backward!`. The iterate is NOT
renormalized — total mass floats with the chain's stages, so an entry/exit (or
reproduction/renorm) chain converges to a Λ whose `Σ(Λ)` is the entry/exit balance,
not forced to 1 (see `population.jl` header). A pure mass-conserving chain keeps its
seed's mass; a constant-population steady state (entry mass = exit mass) has stable
mass on its own.
"""
...
function solve_transition_given_env_path!(spec::ChainStageSpec,
    ...
    for _ in 1:max_inner_iters
        # `env` varies period-to-period along the path, so each step's `backward!` keeps the
        # default `env_changed = true` (a per-period env change) — the §5.3 contract demands a refill
        # whenever env changes, and a fresh per-period chain holds no prior-env seating to skip.
