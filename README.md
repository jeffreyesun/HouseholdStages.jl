...
All four run at `N_w = 400`. Tests: 527 / 527 passing.
...
- **`FromEnv`-keyed swept mode** (`FromEnv(:key)` reading an env field
...
- **`HOWTO_STAGE.md`** — focused quick guide for writing a new
  stage: the six things you author, a worked ArgmaxStage walkthrough,
  common patterns (env-resolved parameters, cached next-state
  indices, the shared softmax helper), and the few footguns.
- **`STAGES_ARCHITECTURE.md`** — design notes: the Spec/Buffer/Stage
  ...
  env-resolution machinery, conventions for stage authors.
