...
@testset "allocate — MarkovStage kernel=nothing, scratch non-empty" begin
    ...
    @test hasproperty(stage.buffer.scratch, :perm_in) && hasproperty(stage.buffer.scratch, :perm_out)
...
@testset "allocate — ChainStage returns per-stage tuple" begin
    ...
    @test hasproperty(chain.buffer.stages[1].scratch, :perm_in)
    @test hasproperty(chain.buffer.stages[2].scratch, :perm_in)
end
