...
@testset "ForgetfulSumStage — drops one axis, 3D → 2D" begin
    ...
    @test layout_size(output_layout(stage)) == (4, 3)
    @test size(stage.buffer.V_start) == (4, 3, 5)
    @test size(stage.buffer.Λ_end)   == (4, 3)
...
@testset "ForgetfulSumStage — drops a middle axis, 4D → 3D" begin
    ...
    @test layout_size(output_layout(stage)) == (3, 4, 5)
