...
@testset "product — view-based fused tensor for uniform MarkovStage" begin
    # With view-stitching (the default), the rebuilt MarkovStage components
    ...
    @test ps.buffer.components[1].V_start isa SubArray
    @test ps.buffer.components[2].V_start isa SubArray
    # The component's V_start is a slice of ps.buffer.V_fused; writing to
    # one should mutate the other.
    fill!(ps.buffer.components[1].V_start, 7.0)
    @test all(ps.buffer.V_fused[:, 1] .== 7.0)
end
...
@testset "product — IdentityStage view-mode" begin
    ...
    ps = product(s1, s2; axis = :group)
    @test ps.buffer.components[1].V_start isa SubArray
    fill!(ps.buffer.components[1].V_start, 3.0)
    @test all(ps.buffer.V_fused[:, 1] .== 3.0)
end
...
@testset "product — ForgetfulSumStage view-mode (V_start views; Λ_end allocates fresh)" begin
    ...
    @test ps.buffer.components[1].Λ_end isa Array
end
...
@testset "product — choice stages now use view-mode too" begin
    ...
    ps = product(s1, s2; axis = :group)
    @test ps.buffer.components[1].V_start isa SubArray
    @test ps.buffer.components[1].Λ_end   isa SubArray
    @test ps.buffer.components[1].kernel.policy isa Array
    # Mutating a component's V_start should reflect in the fused tensor.
    fill!(ps.buffer.components[1].V_start, 9.0)
    @test all(ps.buffer.V_fused[:, 1] .== 9.0)
end
