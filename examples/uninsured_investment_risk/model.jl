...
# `Investment`    — `MeanVarianceStage` on the wealth axis picks the share
#                   `θ ∈ shares` of saved wealth placed in RISKY OWN CAPITAL:
#                   next wealth is `b'·(R_f + θ·(R_k − R_f))` where `R_f` is
#                   the safe store and `R_k` is the agent's idiosyncratic
#                   capital return (a mean-`μ_k` two-point draw). The
#                   idiosyncratic capital risk is exactly `MeanVarianceStage`'s
#                   risky leg — the "portfolio share" is here the capital
#                   share, the variance is undiversifiable production risk.
#
...
# own `R_k`), so `MeanVarianceStage`'s per-cell return distribution is the
# right primitive — no aggregate risk, no diversification.
#
...
@kwdef struct InvestmentRiskParams
    ...
    p_up   :: Float64 = 0.5                        # symmetric two-point draw
...
household stage. The risky leg of the final `MeanVarianceStage` is the
agent's OWN-CAPITAL return — a mean-`μ_k` two-point idiosyncratic draw with
half-spread `Δ` — so `θ` is the share of saved wealth exposed to
undiversifiable capital risk. Raising `Δ` (variance) sharpens the
precautionary tilt away from risky capital.
"""
function investment_risk_household(p = investment_risk_params; Δ = p.Δ)
    ...
    # Own-capital investment: share θ of saved wealth in risky capital with a
    # mean-μ_k, half-spread-Δ idiosyncratic return; the rest earns R_f safe.
    investment = MeanVarianceStage(layout;                                     # defaults: (; axis = :wealth, cost)
        shares        = p.shares,
        risk_free     = p.R_f,
        risky_returns = [p.μ_k - Δ, p.μ_k + Δ],
...
"The `MeanVarianceStage` leaf (the last seated stage) — its seated `policy` is the
...
the `MeanVarianceStage` at build time via `Δ`, so the variance sweep rebuilds
